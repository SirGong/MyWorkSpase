...
public class GetNumberOfKey {
    ...
    public static int getFirstKey(int[] array, int start, int end, int k) {
        ...
        } else if (midData > k) {//中间的值比关键字大，在数组的前半段
            end = midIndex - 1;
        } else {
            start = midIndex + 1;
        }
...
```java
//思路：如果一棵树只有一个节点，那么他的深度为1
//如果它只有左子树，没有右子树，那么他的深度为左子树的深度加一
//如果它只有右子树，没有左子树，那么他的深度为右子树的深度加一
//如果它既有左子树和右子树，那么他的深度为两颗子树深度的较大值加一
public class Solution {
...
可以先将数组进行排序，利用有个栈来进行入栈操作，如果遍历到的当前元素和栈顶元素相同，弹栈
...
解法一充分利用了HashMap的特性，更简单的方法是使用异或运算符对数组进行分割，然后分别寻找
...
public class Solution {
    public ArrayList<Integer> FindNumbersWithSum(int [] array,int sum) {
        ...
        if (array.length < 2 || sum <= 0)
            return res;
        //采用双指针法从两端对数组进行计算，若值要是小于sum，start++，否则end——，直到等于
        int start = 0;
...
public class Solution {
    ...
    public boolean matchCore(char[] str,int s,char[] pattern,int p){
        ...
        //字符串已经匹配到尾部，模式串依然没有匹配完，返回false
        if(s < str.length && p == pattern.length){
            return false;
        }
        //如果p后面的字符指向的是*，此时匹配规则发生改变
        if(p + 1 < pattern.length && pattern[p+1] == '*'){
            ...
            }else{//出现了*，且s和p指向的字符不相同，就当*前的字符出现了0次
                return matchCore(str,s,pattern,p + 2);
            }
        }
        //如果p的后面不是字符*，那就挨个匹配
...
public class Solution {
    public boolean isNumeric(char[] str) {
        if(str == null)
            return false;
        String partten = "^[-+]?\\d*(?:\\.\\d*)?(?:[eE][+\\-]?\\d+)?$";
        String s = new String(str);
        return Pattern.matches(partten,s);
    }
}
```
...
public class Solution {
    public ListNode EntryNodeOfLoop(ListNode pHead){
        ...
        if(meetNode == null)
            return null;
        //计算环中节点的数目
        ListNode curr = meetNode;
        int count = 1;
        while(curr.next != meetNode){
            curr = curr.next;
            count++;
        }
        //第一个指针先从头向前移动节点的个数次
...
思路：针对先序遍历定义一种新的遍历算法，即先遍历根节点，在遍历右子节点，最后遍历左子节点，如果遍历得出的两个序列的内容一致，及该二叉树是对称的。
*/
public class Solution {
...
```java
/**
两个函数分别实现序列化二叉树和反序列化二叉树，空节点用#表示
*/
public class Solution {
    int index = -1;
...
public class Solution {
    ...
    //按照中序遍历将节点的值存入集合
    void midOrder(TreeNode root,ArrayList<TreeNode> list){
        if(root.left != null){
...
public class Solution {
    public int cutRope(int target) {
        ...
        //尽可能的多剪去长度为3的绳子
        int cutOf3 = target / 3;
        //余下长度为4的绳子剪成两个长度为2的小段
        if(target - cutOf3 * 3 == 1)
            cutOf3 -= 1;
        int cutOf2 = (target - cutOf3 * 3) / 2;
        return (int)Math.pow(3,cutOf3)* (int)Math.pow(2,cutOf2);
    }
}
```
